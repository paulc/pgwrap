...
__doc__ = """
    ...
    -------------------
        ...
        Prepared statements can be created using the
            ...
            connection.prepare(stmt,params,name) 

        method where 'stmt' is the prapared statement (with parameters identified
        in the statement using the psql $1,$2... notation), 'params' is a list of optional
        parameter types (usually not needed - infered by psql), and 'name' is the 
        name of the prepared statement (usually autogenerated). The method returns
        a PreparedStatement object which can be used instead of an sql statement in 
        the connection.execute and connection.query methods.
